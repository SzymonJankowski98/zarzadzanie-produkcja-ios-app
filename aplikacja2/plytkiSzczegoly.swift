...
class plytkiSzczegoly: UIViewController, UITableViewDataSource, UITableViewDelegate, maszynyDelegate
{
    //var id_narzedzia = ""
    ...
    @IBAction func zuzycieAction(sender: AnyObject) {
        ...
        let zapytanie = "a=UPDATE `plytki_ostrza` SET `zuzycie` = '\(temp)' WHERE `plytki_ostrza`.`ID_plytki` = \(id_plytki2) AND `plytki_ostrza`.`Nr_ostrza` = \(sender.tag);"
        print(zapytanie)
    ...
    @IBAction func ilosc_akcja(sender: UITextField) {
        var zapisz_maszyna = zapisz()
        let path = "http://localhost/narzedzia_zapisz.php"
        let zapytanie = "a=UPDATE `plytki_ostrza` SET `Ilosc_sztuk` = '\(sender.text!)' WHERE `plytki_ostrza`.`ID_plytki` = \(id_plytki2) AND `plytki_ostrza`.`Nr_ostrza` = \(sender.tag);"
        print(zapytanie)
        zapisz_maszyna.zapisz(zapytanie, url: path)
    }
    ...
    @IBAction func stepper_action(sender: UIStepper) {
        ...
        var zapisz_maszyna = zapisz()
        let path = "http://localhost/narzedzia_zapisz.php"
        let zapytanie = "a=UPDATE `plytki_ostrza` SET `Ilosc_sztuk` = '\(cell.ilosc_text.text!)' WHERE `plytki_ostrza`.`ID_plytki` = \(id_plytki2) AND `plytki_ostrza`.`Nr_ostrza` = \(sender.tag);"
        print(zapytanie)
        zapisz_maszyna.zapisz(zapytanie, url: path)
    }
    ...
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        var adres = "http://localhost/plytki.php?id=\(id_plytki2)"
        print(adres)
